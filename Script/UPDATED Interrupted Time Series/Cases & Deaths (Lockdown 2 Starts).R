...
UK <- filter(owid , location=="United Kingdom"& date>="2020-10-15" & date <= "2020-11-26")

UK$Dayno <-data.frame(Dayno = rep(c(1:43)))

# Create new variable that groups pre-lockdown and post-lockdown 
UK$lockdown <- c(if_else(UK$date <"2020-11-05", 0, 1))
...
ggplot(UK, aes(x=date)) +
  ...
  geom_vline(xintercept = as.numeric(as.Date("2020-11-05")), linetype=2, colour = "firebrick2", lwd = 0.5) +
  annotate("text", x=as.Date("2020-11-05"), y=600, label= "Second Lockdown Begins (05 Nov)", angle=90, vjust = 1.5, colour = "firebrick2", fontface = 'bold')+
  ggtitle("Weekly count of confirmed case and death numbers in the UK", subtitle = "(Lockdown 2 Begins)") +
...
# Save Plot
ggsave("Cases&Deaths_Lockdown 2 Starts.png",width = 10, limitsize = FALSE, path = Interrupred_Time_Series_Graphs.dir)
