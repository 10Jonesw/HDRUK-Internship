...
UK <- filter(owid , location=="United Kingdom"& date>="2020-03-06" & date <= "2020-04-17")
...
UK <- UK %>% mutate(Lockdownday = UK$Dayno-21, change=ifelse(Dayno>=21, 1, 0))
...
ggplot(UK, aes(x=date)) +
  ...
  ggtitle("Weekly count of confirmed case and death numbers in the UK", subtitle = "(Lockdown 1 Begins)") +
...
# Save Plot
ggsave("Cases&Deaths_Lockdown 1 Starts.png",width = 10, limitsize = FALSE, path = Interrupred_Time_Series_Graphs.dir)
