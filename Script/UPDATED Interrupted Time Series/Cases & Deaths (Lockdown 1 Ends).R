...
UK <- filter(owid , location=="United Kingdom"& date>="2020-05-14" & date <= "2020-06-25")

UK$Dayno <-data.frame(Dayno = rep(c(1:43)))

# Create new variable that groups pre-lockdown and post-lockdown 
UK$lockdown <- c(if_else(UK$date <"2020-06-01", 0, 1))
...
UK <- UK %>% mutate(Lockdownday = UK$Dayno-19, change=ifelse(Dayno>=19, 1, 0))
...
ggplot(UK, aes(x=date)) +
  ...
  annotate("text", x=as.Date("2020-06-01"), y=50, label= "First Lockdown Eases (1 Jun)", angle=90, vjust = 1.5, colour = "firebrick2", fontface = 'bold')+
  ggtitle("Weekly count of confirmed case and death numbers in the UK", subtitle = "(Lockdown 1 Eases)") +
...
# Save Plot
ggsave("Cases&Deaths_Lockdown 1 Ends.png",width = 10, limitsize = FALSE, path = Interrupred_Time_Series_Graphs.dir)
