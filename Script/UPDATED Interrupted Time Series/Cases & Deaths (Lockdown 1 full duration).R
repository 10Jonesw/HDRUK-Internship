...
UK <- filter(owid , location=="United Kingdom"& date>="2020-03-06" & date <= "2020-06-25")

UK$Dayno <-data.frame(Dayno = rep(c(1:112)))

# Create new variable that groups pre-lockdown and post-lockdown 
UK$lockdown <- c(if_else(UK$date <"2020-03-26", 0, 1))
...
ggplot(UK, aes(x=date)) +
  ...
  geom_vline(xintercept = as.numeric(as.Date("2020-03-26")), linetype=2, colour = "firebrick2", lwd = 0.5) +
  annotate("text", x=as.Date("2020-03-26"), y=8, label= "First Lockdown Begins (26 Mar)", angle=90, vjust = 1.5, colour = "firebrick2", fontface = 'bold')+
  geom_vline(xintercept = as.numeric(as.Date("2020-06-01")), linetype=2, colour = " firebrick2", lwd = 0.5)+
  annotate("text", x=as.Date("2020-06-01"), y=8, label= "First Lockdown Eases (1 Jun)", angle=90, vjust = 1.5, colour = "firebrick2", fontface = 'bold')+
  ggtitle("Weekly count of confirmed case and death numbers in the UK", subtitle = "(Lockdown 1)") +
...
# Save Plot
ggsave("Cases&Deaths_Lockdown 1 Full duration.png",width = 10, limitsize = FALSE, path = Interrupred_Time_Series_Graphs.dir)
