...
UKdates <- filter(UKdates , date>="2020-03-06")
UK <- cbind(UK, UKdates)
...
ggplot(UK, aes(x=date)) +
  ...
  scale_y_log10() +
  scale_x_date(date_breaks = "1 week", date_labels = "%d-%b-%Y", limits = as.Date(c('2020-01-30','2020-07-30')))+
  ...
  ggtitle("Weekly count of COVID Deaths in the UK", subtitle = "Plot for 8 weeks before and after lockdown") +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))

# Save Plot
ggsave("INTERRUPTED TIME SERIES PLOT_Deaths_8 Weeks (Lockdown 1).png",width = 15, limitsize = FALSE, path = Interrupred_Time_Series_Graphs.dir)
